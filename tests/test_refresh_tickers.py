...
def test_load_symbols_for_refresh_from_db_raises_when_empty() -> None:
    with patch("refresh_tickers._load_db_symbols", return_value=[]):
        with pytest.raises(
            ValueError, match="No tickers found in us_tickers, swe_tickers, or uk_tickers"
        ):
            load_symbols_for_refresh(
                "postgresql://example",
                tickers_path=Path("tickers.txt"),
...
def test_load_db_symbols_uses_country_tickers_loader() -> None:
    ...
    with patch(
        ...
        assert _load_db_symbols("postgresql://example") == [
            "AAPL",
            "VOLV-A.ST",
            "VOD.L",
        ]
...
def test_main_refresh_resolves_listing_market_and_exchange_name() -> None:
    mock_config = type(
        "Cfg",
        (),
        {
...
def test_main_refresh_uk_symbol_passes_exchange_name() -> None:
    mock_config = type(
        "Cfg",
        (),
        {
...
def test_build_parser_mentions_uk_and_exchange_name() -> None:
    from refresh_tickers import build_parser

    parser = build_parser()
    help_text = parser.format_help()
    assert "uk_tickers" in help_text
    assert "exchange_name" in help_text
