...
Data is partitioned by listing country into two parallel table sets:
...
`schema.sql` must create the US and Swedish table sets above (PRD §6). Then seed the watchlists (`pipenv run python seed_tickers.py`) before running fetch or backfill jobs.
...
Steps 1–10 upgrade the **legacy** single-set tables (`tickers` / `metrics` / `market` → `market_metrics`). Step 11 splits that layout into the country-specific sets.
...
| 11 | `migrate_split_us_swe_tables.sql` | Create `us_*` / `swe_*` table sets; move rows from legacy `tickers` / `metrics` / `market_metrics` by listing country; drop legacy tables (PRD §6) |
...
**Fresh install (no tables):** `schema.sql` only (US + Swedish table sets).
...
Used by `.github/workflows/dev-backfill.yml` against the Neon **dev** branch:
...
3. Always run step 11 (`migrate_split_us_swe_tables.sql`).

Fresh country-only databases skip the legacy chain and only get `schema.sql` + step 11.
...
Or run the queries in the Neon SQL editor. Verification should assert the US and Swedish table sets (`us_tickers`, `us_metrics`, `us_market_metrics`, `swe_tickers`, `swe_metrics`, `swe_market_metrics`) once step 11 / updated `schema.sql` are in place.
